...
def ingurgitate_crypto(plantoid, network, tID, amount):
    ...
    style = "Contemporary art song, post-human female vocal performance with lyrics, austere and minimal female voice, slow contemplative delivery, sacred contemporary classical, dark ambient liturgical, synthetic feminine voice as artistic medium, in the lineage of Holly Herndon and Laurie Anderson, no instruments, voice and silence only, gallery installation aesthetic, slow ceremonial tempo"
...
def create_seed_metadata(plantoid, network, tID):
    ...
    behaviors.generic_metadata(plantoid, network, tID, db, None,
        behaviors.glitchbox_build_video_scheduler(
            lora="artfullytextual-xl,disco-diffusion-xl",
            fps=5,
            strength=0.7,
            cn_scale=0.5,
            video_set="ballerina_shutterstock",
            audio_band="bass",
        ),
        audio_merge=False,
    )
